...
function Actions(team: TeamInterface): JSX.Element {
    ...
    return (
        <div className={`h-full flex items-center justify-center ${containerFlexDirection}`}>
            ...
            <div onClick={switchPossessionStatus} className={`h-14 w-32 rounded-md flex items-center justify-center cursor-pointer select-none text-l bg-green-200 ${possessionMargin}`}>
                {t('possession')}
            </div>
        </div>
    )
